...
texts = ["Original_HLSU_data_half_step_rolling"]
frame_distances = ["rolling_1_step"]
video_path = Path('/workspaces/TimeSFormer/timesformer/datasets/hslu/gen2r')
configs = [
    '/workspaces/TimeSFormer/configs/hslu/TimeSformer_divST_8_224_gen2_half_step_rolling.yaml']
...
for config,model_file,model_name,frame_distance  in zip(configs,model_files,texts,frame_distances): 
    ...
    for frame_index in tqdm(range(0,50)):#len(hslu_dataset))):#len(10))):#))hslu_dataset))):
