...
def combine_temporal_attention(attn_t_full, head_fuse_method='mean'): 
    patches,heads,h,w = attn_t_full[0].shape
    initial_I = torch.eye(h,w)
    joint_attentions = torch.zeros((patches,h,w)).to(attn_t_full[0].device)
    for index,encoder_attn_t in enumerate(attn_t_full):
      encoder_attn_t = fuse_attention_heads(encoder_attn_t,head_fuse_method)
      encoder_attn_t = encoder_attn_t +  torch.eye(encoder_attn_t.size(-1))[None,...]
      encoder_attn_t = encoder_attn_t / encoder_attn_t.sum(-1)[...,None]

      for patch in range(0,patches):
        if index==0:# The first encoder is multiplied with the Indentity, the rest is using its 
          joint_attentions[patch] = torch.matmul(encoder_attn_t[patch],initial_I)
        else:
          joint_attentions[patch] = torch.matmul(encoder_attn_t[patch],joint_attentions[patch])

    masks = []
    img_width = int(patches**0.5)
    for patch in range(0,patches):
      mask = joint_attentions[patch,0,:].numpy()
      # mask_1 = joint_attentions[patch,:,0].numpy() # looks differently
      mask = mask / np.max(mask)
      masks.append(np.expand_dims(mask,0))
    return np.vstack(masks).reshape(img_width,img_width,h)

def combine_spatial_attention(attn_s_full,head_fuse_method='mean'): 
    imgs,heads,h,w = attn_s_full[0].shape
    initial_I = torch.eye(h,w)
    joint_attentions = torch.zeros((imgs,h,w)).to(attn_s_full[0].device)
    for index,encoder_attn_s in enumerate(attn_s_full):
      encoder_attn_s = fuse_attention_heads(encoder_attn_s,head_fuse_method)
      encoder_attn_s = encoder_attn_s +  torch.eye(encoder_attn_s.size(-1))[None,...]
      encoder_attn_s = encoder_attn_s / encoder_attn_s.sum(-1)[...,None]
      for img in range(0,imgs):
        if index == 0: # the first encoder
          joint_attentions[img] = torch.matmul(encoder_attn_s[img],initial_I)
        else:
          joint_attentions[img] = torch.matmul(encoder_attn_s[img],joint_attentions[img])   
    masks = []
    for img in range(0,imgs):
      mask = joint_attentions[img,0,1:] # Check attention between the class token 1:
...
class DividedAttentionRollout():
  ...
  def __call__(self, input_tensor):
    ...
    self.attentions = []
    # self.attentions_space_mean=[]
    # self.attentions_temporal_mean=[]
    self.attentions_min = []
    self.attentions_max = []
    ...
    self.attentions_space_mean = combine_spatial_attention( self.space_attentions)
    self.attentions_temporal_mean = combine_temporal_attention( self.time_attentions)
    for attn_t,attn_s in zip(self.time_attentions, self.space_attentions):
    ...
    def generate_mask(attention,index=0):
        ...
        return mask
    return (generate_mask(self.attentions),generate_mask(self.attentions_min),generate_mask(self.attentions_max)),preds,(self.attentions_space_mean,self.attentions_temporal_mean)
