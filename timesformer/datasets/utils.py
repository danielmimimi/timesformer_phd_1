...
def simple_scale(frames,crop_size=224):

    transposed_tensor = frames.permute(1, 0, 2, 3)

    resized_tensor = F.interpolate(transposed_tensor, size=(crop_size, crop_size), mode='bilinear', align_corners=False)
    #first_image = resized_tensor[0]
    final_tensor = resized_tensor.permute(1, 0, 2, 3)
