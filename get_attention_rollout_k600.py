...
texts = ["HSLU_Real","Base_K600"]
frame_distances = ["uniform","uniform"]

# video_path = Path('/workspaces/TimeSFormer/timesformer/datasets/k600/unpacked')

configs = [
    '/workspaces/TimeSFormer/configs/hslu/TimeSformer_divST_8_224_gen2r.yaml',
    '/workspaces/TimeSFormer/configs/hslu/TimeSformer_divST_8x32_224_k600.yaml'
    ]


output_dir = Path('/workspaces/TimeSFormer/original_trained_k600')
...
for config,model_file,model_name,frame_distance  in zip(configs,model_files,texts,frame_distances): 
    ...
    model_name =  model_name+"_{}".format(frame_distance)
    ...
    html_content += '<li><a href="#confusion-matrix">Confusion Matrix</a></li>'
    html_content += '<li><a href="#accuracy">Accuracy</a></li>'
    html_content += '<li><a href="#negative-evaluation">Negative Samples</a></li>'
    html_content += '<li><a href="#positive-evaluation">Positive Samples</a></li>'
    html_content += "</ul>"
    ...
    html_content += f'<h2 id="confusion-matrix">Confusion Matrix for {model_name}</h2>'
    ...
    for frame_index in tqdm(range(0,len(hslu_dataset))):#len(10))):#))hslu_dataset))):
        ...
        # Draw grid on the first image
        # first_image_np = np_imgs[0]
        # first_image = Image.fromarray((first_image_np * 255).astype(np.uint8))
        # grid_image = draw_grid(first_image, 14)
        # grid_image_np = np.array(grid_image)
        # np_imgs[0] = grid_image_np 
        ...
        if class_of_prediction == label:
            tp.append({'label':label,'prediction':class_of_prediction,"image":relative_combined_image_path,'folder_name':folder_name})
        else:
            fn.append({'label':label,'prediction':class_of_prediction,"image":relative_combined_image_path,'folder_name':folder_name})
    ...
    html_content += f'<h2 id="negative-evaluation">Negative Sample</h2>'
    ...
    html_content += f'<h2 id="positive-evaluation">Positive Sample</h2>'
    ...
    html_content += f'<h3 id="accuracy">Accuracy for {model_name}: {accuracy:.2f}</h3>'
